...
/**
 * Release script for claude-recall
...
async function publish() {
  try {
    console.log('📦 claude-recall Marketplace Release Tool\n');
    ...
    const marketplaceJson = JSON.parse(fs.readFileSync('.claude-extension/marketplace.json', 'utf-8'));
    marketplaceJson.plugins[0].version = newVersion;
    fs.writeFileSync('.claude-extension/marketplace.json', JSON.stringify(marketplaceJson, null, 2) + '\n');
    ...
    await execAsync('git add package.json .claude-extension/marketplace.json extension/');
    await execAsync(`git commit -m "chore: Release v${newVersion}

Marketplace release for Claude Code plugin
https://github.com/nhevers/claude-recall"`);
    ...
    console.log(`\n🏷️  Tag: https://github.com/nhevers/claude-recall/releases/tag/v${newVersion}`);
