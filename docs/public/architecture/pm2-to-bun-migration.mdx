...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing claude-recall for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
claude-recall version 7.1.0 introduces two major architectural migrations:
...
- **Process Name**: `claude-recall-worker`
...
- **Logs**: `~/.pm2/logs/claude-recall-worker-*.log`
- **PID File**: `~/.pm2/pids/claude-recall-worker.pid`

**Lifecycle Commands**:
```bash
pm2 start <script>           # Start worker
pm2 stop claude-recall-worker   # Stop worker
pm2 restart claude-recall-worker # Restart worker
pm2 delete claude-recall-worker  # Remove from PM2
pm2 logs claude-recall-worker    # View logs
```
...
**Component**: Custom ProcessManager (`src/core/process/ProcessManager.ts`)
...
- **Port File**: `~/.claude-recall/.worker.port` (new)
...
- Database file: `~/.claude-recall/claude-recall.db` (same location)
...
if (!existsSync(pm2MigratedMarker)) {
  try {
    spawnSync('pm2', ['delete', 'claude-recall-worker'], { stdio: 'ignore' });
...
`ensureWorkerRunning()` checks if `~/.claude-recall/.worker.pid` exists (it doesn't for first run after update)
</Step>
<Step title="Start Worker Decision">
Worker not running → Call `startWorker()`
</Step>
<Step title="Migration Check">
Check if `~/.claude-recall/.pm2-migrated` exists
</Step>
<Step title="PM2 Cleanup">
Execute `pm2 delete claude-recall-worker` (errors ignored), create marker file
</Step>
...
**Location**: `~/.claude-recall/.pm2-migrated`
...
4. **PM2 cleanup**: `pm2 delete claude-recall-worker` (old worker terminated)
5. **Marker creation**: `~/.claude-recall/.pm2-migrated` with timestamp
...
| `pm2 stop claude-recall-worker` | `npm run worker:stop` | Stop worker |
| `pm2 restart claude-recall-worker` | `npm run worker:restart` | Restart worker |
| `pm2 delete claude-recall-worker` | `npm run worker:stop` | Remove worker |
| `pm2 logs claude-recall-worker` | `npm run worker:logs` | View logs |
| `pm2 describe claude-recall-worker` | `npm run worker:status` | Detailed status |
...
**Logs**:
```
Old: ~/.pm2/logs/claude-recall-worker-out.log
     ~/.pm2/logs/claude-recall-worker-error.log

New: ~/.claude-recall/logs/worker-YYYY-MM-DD.log
```

**PID Files**:
```
Old: ~/.pm2/pids/claude-recall-worker.pid

New: ~/.claude-recall/.worker.pid
```
...
New: ~/.claude-recall/.worker.pid
     ~/.claude-recall/.worker.port
     ~/.claude-recall/.pm2-migrated (all platforms)
```

**Database** (unchanged):
```
Same: ~/.claude-recall/claude-recall.db
```
...
├────┼────────────────────┼─────────┼─────────┼──────────┤
│ 0  │ claude-recall-worker  │ online  │ 0       │ 2d 5h    │
└────┴────────────────────┴─────────┴─────────┴──────────┘
```
...
# Or just remove claude-recall logs
rm -f ~/.pm2/logs/claude-recall-worker-*.log
rm -f ~/.pm2/pids/claude-recall-worker.pid
```
...
```
~/.claude-recall/
├── claude-recall.db          # Database (unchanged)
...
~/.pm2/
├── logs/
│   ├── claude-recall-worker-out.log
│   └── claude-recall-worker-error.log
├── pids/
│   └── claude-recall-worker.pid
└── pm2.log
```
...
```
~/.claude-recall/
├── claude-recall.db          # Database (same file)
├── chroma/                # Vector embeddings (unchanged)
├── logs/
...
**Symptoms**:
- `pm2 list` still shows `claude-recall-worker`
...
```bash
# Manual cleanup
pm2 delete claude-recall-worker
pm2 save  # Persist the deletion

# Force re-migration (optional)
rm ~/.claude-recall/.pm2-migrated
...
**Manual Resolution**:
```bash
rm ~/.claude-recall/.worker.pid
rm ~/.claude-recall/.worker.port
...
| `Invalid port X` | Port validation failed | Update `CLAUDE_RECALL_WORKER_PORT` in settings |
...
# 2. Start PM2 worker
pm2 start extension/runtime/engine-cli.js --name claude-recall-worker
...
# 4. Trigger hook
node extension/runtime/session-start-hook.js

# 5. Verify migration
pm2 list  # Should NOT show claude-recall-worker
cat ~/.claude-recall/.pm2-migrated  # Should exist
