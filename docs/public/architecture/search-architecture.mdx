...
claude-recall uses an **MCP-based search architecture** that provides intelligent memory retrieval through 4 streamlined tools following a 3-layer workflow pattern.
...
2. **MCP Server** (`extension/runtime/search-server.cjs`) - Thin wrapper over HTTP API
...
**Location:** `/Users/YOUR_USERNAME/.claude/plugins/marketplaces/nhevers/extension/runtime/search-server.cjs`
...
The 3-layer workflow embodies **progressive disclosure** - a core principle of claude-recall's architecture.
...
2. Check database: `ls ~/.claude-recall/claude-recall.db`
