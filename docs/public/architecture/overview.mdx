...
description: "System components and data flow in claude-recall"
---
...
claude-recall operates as a Claude Code plugin with five core components:
...
5. **history-search Skill** - Skill-based search with progressive disclosure (v5.4.0+)
...
│   │   ├── summary-hook.js
│   │   ├── cleanup-hook.js
│   │   └── engine-runtime.cjs  # Background worker + HTTP API
│   │
│   ├── skills/                 # Agent skills (v5.4.0+)
│   │   ├── history-search/         # Search skill with progressive disclosure (v5.5.0)
...
**Note**: smart-install.js is a pre-hook dependency checker (not a lifecycle hook). It's called before context-hook via command chaining in lifecycle.json and only runs when dependencies need updating.
...
**Skill Enhancement (v5.5.0)**: Renamed from "search" to "history-search" for better scope differentiation. Effectiveness increased from 67% to 100% with enhanced triggers and comprehensive documentation.
