...
description: Complete reference for integrating claude-recall worker service into VSCode extensions, IDE plugins, and CLI tools
icon: plug
---

<Note>
**Version:** 7.0.0 (December 2025)
**Target Audience:** Developers building claude-recall integrations (VSCode extensions, IDE plugins, CLI tools)
</Note>
...
const DEFAULT_PORT = 37777; // Override with CLAUDE_RECALL_WORKER_PORT
```
...
```bash
CLAUDE_RECALL_MODEL=claude-sonnet-4-5          # Model for observations/summaries
CLAUDE_RECALL_CONTEXT_OBSERVATIONS=50          # Observations injected at SessionStart
...
**Tag Stripping:** Removes `<private>` and `<claude-recall-context>` tags before storage.
...
```json Response
{
  "projects": ["claude-recall", "other-project", ...]
}
```
...
```http
GET /api/search/by-file?filePath=src/core/worker-service.ts&format=index&limit=10
```
...
// Register tool
vscode.lm.registerTool('claude-recall-search', searchTool);
```

#### Chat Participant Implementation

```typescript
const participant = vscode.chat.createChatParticipant('claude-recall', async (request, context, stream, token) => {
...
- `<claude-recall-context>content</claude-recall-context>` - System-level tag (prevents recursive storage)
...
class WorkerUnavailableError extends Error {
  constructor() {
    super('claude-recall worker is not running or unreachable');
...
```json
{
  "name": "claude-recall-vscode",
  "displayName": "claude-recall",
  "version": "1.0.0",
  "engines": {
    "vscode": "^1.95.0"
  },
...
describe('Worker Integration', () => {
  ...
  beforeAll(async () => {
    ...
    workerProcess = spawn('node', ['dist/worker-service.js'], {
      env: { ...process.env, CLAUDE_RECALL_WORKER_PORT: '37778' }
    });
...
export function registerSearchTool(context: vscode.ExtensionContext) {
  ...
  const searchTool = vscode.lm.registerTool('claude-recall-search', {
    description: 'Search persistent memory for observations, sessions, and prompts',
    inputSchema: {
      type: 'object',
      properties: {
...
<CardGroup cols={2}>
  <Card title="Documentation" icon="book" href="https://claude-recall.ai">
    Complete claude-recall documentation
  </Card>
  <Card title="GitHub" icon="github" href="https://github.com/nhevers/claude-recall">
