...
description: "How claude-recall evolved from v3 to v5+"
---
...
This is the story of how claude-recall evolved from a simple idea to a production-ready system, and the key architectural decisions that made it work.
...
const ThemeProvider = ({ children }) => {
  const [theme, setTheme] = useState<'light' | 'dark' | 'system'>(() => {
    return localStorage.getItem('claude-recall-theme') || 'system';
  });

  useEffect(() => {
    localStorage.setItem('claude-recall-theme', theme);
  }, [theme]);
...
**Impact**: Cross-platform compatibility restored, Windows users can now use claude-recall without issues.
...
esbuild.build({
  ...
  bundle: true,
  outfile: 'extension/web/viewer.html',
...
- history-search skill invoked via natural language
...
- All 19 history-search skill files removed (~2,744 lines)
...
```bash
cp ~/.claude-recall/claude-recall.db ~/.claude-recall/claude-recall-v3-backup.db
```
