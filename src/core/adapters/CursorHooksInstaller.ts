...
export function findCursorHooksDir(): string | null {
  const possiblePaths = [
    ...
    path.join(homedir(), '.claude', 'plugins', 'marketplaces', 'nhevers', 'cursor-hooks'),
    // Development/source location (relative to built engine-runtime.cjs in extension/runtime/)
...
export function findMcpServerPath(): string | null {
  const possiblePaths = [
    // Marketplace install location
    path.join(homedir(), '.claude', 'plugins', 'marketplaces', 'nhevers', 'plugin', 'scripts', 'search-server.cjs'),
    // Development/source location (relative to built engine-runtime.cjs in extension/runtime/)
    path.join(path.dirname(__filename), 'search-server.cjs'),
    // Alternative dev location
    path.join(process.cwd(), 'plugin', 'scripts', 'search-server.cjs'),
  ];
...
/**
 * Find engine-runtime.cjs path for unified CLI
 * Searches in order: marketplace install, source repo
 */
export function findWorkerServicePath(): string | null {
  const possiblePaths = [
    // Marketplace install location
    path.join(homedir(), '.claude', 'plugins', 'marketplaces', 'nhevers', 'plugin', 'scripts', 'engine-runtime.cjs'),
    // Development/source location (relative to built engine-runtime.cjs in extension/runtime/)
    path.join(path.dirname(__filename), 'engine-runtime.cjs'),
    // Alternative dev location
    path.join(process.cwd(), 'plugin', 'scripts', 'engine-runtime.cjs'),
  ];
...
export function configureCursorMcp(target: CursorInstallTarget): number {
  ...
  if (!mcpServerPath) {
    ...
    console.error('   Expected at: ~/.claude/plugins/marketplaces/nhevers/extension/runtime/search-server.cjs');
    return 1;
  }
...
export async function installCursorHooks(_sourceDir: string, target: CursorInstallTarget): Promise<number> {
  ...
  // Find the engine-runtime.cjs path
  const workerServicePath = findWorkerServicePath();
  if (!workerServicePath) {
    console.error('Could not find engine-runtime.cjs');
    console.error('   Expected at: ~/.claude/plugins/marketplaces/nhevers/extension/runtime/engine-runtime.cjs');
    return 1;
  }
  ...
  try {
    ...
    // Generate lifecycle.json with unified CLI commands
    const hooksJsonPath = path.join(targetDir, 'lifecycle.json');

    // Use the absolute path to engine-runtime.cjs
    ...
    console.log(`  Created lifecycle.json (unified CLI mode)`);
...
Hooks installed to: ${targetDir}/lifecycle.json
Using unified CLI: node engine-runtime.cjs hook cursor <command>
...
export function uninstallCursorHooks(target: CursorInstallTarget): number {
  ...
  try {
    ...
    // Remove lifecycle.json
    if (existsSync(hooksJsonPath)) {
      unlinkSync(hooksJsonPath);
      console.log(`  Removed lifecycle.json`);
    }
...
export function checkCursorHooksStatus(): number {
  ...
  for (const loc of locations) {
    const hooksJson = path.join(loc.dir, 'lifecycle.json');
    ...
    if (existsSync(hooksJson)) {
      ...
      try {
        ...
        if (firstCommand.includes('engine-runtime.cjs') && firstCommand.includes('hook cursor')) {
          console.log(`   Mode: Unified CLI (node engine-runtime.cjs)`);
        } else {
        ...
        console.log(`   Mode: Unable to parse lifecycle.json`);
      }
  ...
  switch (subcommand) {
    case 'install': {
      ...
      if (!cursorHooksDir) {
        ...
        console.error('   Expected at: ~/.claude/plugins/marketplaces/nhevers/integrations/cursor/');
        return 1;
      }
