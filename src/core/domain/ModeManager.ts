...
import { getPackageRoot } from '../../common/paths.js';
...
export class ModeManager {
  ...
  private constructor() {
    // Modes are in extension/profiles/
    // getPackageRoot() points to extension/ in production and src/ in development
    // We want to ensure we find the modes directory which is at the project root/extension/modes
    const packageRoot = getPackageRoot();
    
    // Check for extension/modes relative to package root (covers both dev and prod if paths are right)
    const possiblePaths = [
      join(packageRoot, 'modes'),           // Production (extension/modes)
      join(packageRoot, '..', 'plugin', 'modes'), // Development (src/../extension/modes)
    ];
